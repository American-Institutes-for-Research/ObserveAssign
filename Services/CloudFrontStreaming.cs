...
namespace ObserveAssign.Services
{
    ...
    /// From - http://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/CreateSignatureInCSharp.html
    /// </summary>
    public class CloudFrontStreaming
    {
        /// <summary>
        /// Gets a signed URL for a video in a private S3 bucket using CloudFront
        /// </summary>
        /// <param name="urlString">The full URL path to the file (e.g. https://cloudfrontURL/fileName.extension )</param>
        /// <returns></returns>
        public static string getSignedURL(string urlString, IConfiguration configuration)
        {
            string privateAccessKey = configuration["AWSS3PrivateAccessKey"];
            string cloudFrontURL = configuration["AWSCloudFrontURL"];
            string pathToPrivateKey = configuration["pathToPrivateKey"];
            string privateKeyId = configuration["AWSCloudFrontAccessID"];

            string URL = string.Empty;

            //Example URL String: "https://d2fgy0175923bg.cloudfront.net/test-video-private.mp4";
            urlString = cloudFrontURL + urlString;

            StreamReader txtReader = new StreamReader(pathToPrivateKey);
            DateTime expiresOn = DateTime.Now.AddHours(24);

            //before sending request - set pre-flight information -- https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS            
